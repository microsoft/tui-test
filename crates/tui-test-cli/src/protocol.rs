...
pub enum Request {
    ...
    Open {
        ...
        #[serde(default)]
        style: Box<tui_test::render::style::Style>,
        cols: u16,
        rows: u16,
        cwd: Option<String>,
    ...
    pub fn execute(self, engine: &Engine) -> Response {
        ...
        } = self
        {
            ...
            if context.trace.is_none() {
                if let Request::Open { trace, .. } = request.as_ref() {
        ...
        let trace = match &self {
            Self::Open { trace, .. } => Some(trace.clone()),
            _ => None,
        };
...
mod tests {
    ...
    fn make_open_req(wait_ready: Option<bool>, timeouts: Timeouts) -> Request {
        Request::Open {
            ...
            trace: tui_test::TraceOptions::default(),
    ...
    fn responses_preserve_structured_failure_details() {
        let details =
            FailureDetails::new("locator.expect", FailureReason::LocatorNoMatch, "missing");
        let response =
        ...
        assert_eq!(value["details"]["operation"], "locator.expect");
        assert!(value["details"].get("terminal").is_none());
    }
