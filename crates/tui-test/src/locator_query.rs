...
impl LocatorExpression {
    pub fn into_query(self) -> Result<LocatorQuery, TuiTestError> {
        ...
        let mut queries: Vec<LocatorQuery> = Vec::new();
        ...
        for (index, node) in self.nodes.into_iter().enumerate() {
            ...
            let required =
                |reference: Option<usize>, name: &str| -> Result<Box<LocatorQuery>, TuiTestError> {
                    reference
                        .map(|i| Box::new(queries[i].clone()))
                        .ok_or_else(|| error(&format!("requires {name}")))
                };
            let selector = match node.kind {
                LocatorNodeKind::Text => LocatorSelector::Text(TextSelector {
                    text: node
                        .text
                        .ok_or_else(|| error("text locator requires text"))?,
                ...
                LocatorNodeKind::Link => LocatorSelector::Link(LinkSelector {
                    uri: node
                        .link
                        .ok_or_else(|| error("link locator requires link"))?,
                    full: node.full.unwrap_or(false),
                }),
                LocatorNodeKind::And => LocatorSelector::And {
                    left: required(node.left, "left")?,
                    right: required(node.right, "right")?,
                },
                ...
                LocatorNodeKind::Filter => {
                    ...
                    LocatorSelector::Filter {
                        input: required(node.input, "input")?,
                        has: node.has.map(|i| Box::new(queries[i].clone())),
                        has_not: node.has_not.map(|i| Box::new(queries[i].clone())),
                    }
                }
            };
            ...
            queries.push(LocatorQuery {
                occurrence: node.occurrence,
                within: node.within.map(|i| Box::new(queries[i].clone())),
                direction,
        ...
        Ok(queries.swap_remove(self.root))
    }
}
