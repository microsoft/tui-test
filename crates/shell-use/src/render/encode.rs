...
pub(crate) fn encode(
    ...
    match format {
        RecordingFormat::Apng => encode_apng(path, frames, renderer, cols),
        RecordingFormat::Gif => encode_gif(path, frames, renderer, cols),
...
fn encode_gif(
    ...
    for step in gif_timeline(frames) {
        let mut pixels = renderer.render(&frames[step.frame].grid, cols)?.into_raw();
...
fn encode_apng(
    ...
    for frame in frames {
        let image = renderer.render(&frame.grid, cols)?;
...
mod tests {
    ...
    fn sub_centisecond_frames_are_coalesced_without_zero_delays() {
        let frames = (0..4)
            .map(|index| Frame {
                grid: sample_grid(Color::Rgb(index, 0, 0)),
    ...
    fn apng_and_gif_round_trip_dimensions_delays_and_color() {
        for format in [RecordingFormat::Apng, RecordingFormat::Gif] {
            ...
            encode(&path, format, &frames, &mut renderer, 1).unwrap();
    ...
    fn repeated_apng_encodes_are_byte_identical() {
        ...
        for path in [&first, &second] {
            ...
            encode(path, RecordingFormat::Apng, &frames, &mut renderer, 1).unwrap();
        }
        assert_eq!(
