...
Pass `background="#rrggbb"` or `transparent=True` to `screenshot()` and `start_recording()` to customize the capture canvas. Transparency is available for SVG, APNG, and GIF.
