...
| `click text "T" [selector/style options]`                                  | Auto-wait for one match and click its middle cell; accepts the mouse button and modifier flags. |
...
middle cell; it also accepts `--button left|middle|right` and `--alt`,
`--ctrl`, or `--shift`. `highlight text` marks every selected match in the live
