...
async function pauseClock(page) {
  // Installing the clock alone still lets real time elapse between browser actions.
...
test("zoom and responsive fit retain the same cell hit testing and crop", async ({ page, openReport }) => {
  ...
  data.timeline.frames[2].svg = `<svg xmlns="http://www.w3.org/2000/svg" width="814" height="502" viewBox="0 0 814 502">
    <rect width="100%" height="100%" fill="#ff00ff"/>
    <rect x="7" y="11" width="800" height="480" fill="#112233"/>
    <rect x="7" y="11" width="10" height="20" fill="#ff0000"/>
  </svg>`;
  await openReport(data);
  const crop = await page.locator("#screen-image").evaluate(async (image) => {
    await image.decode();
    ...
    return {
      ...
      firstCellStart: pixel(0, 0), firstCellEnd: pixel(9, 19),
      nextCell: pixel(10, 0), lastCell: pixel(799, 479),
    };
  });
  expect(crop).toEqual({
    width: 800, height: 480,
    firstCellStart: [255, 0, 0, 255], firstCellEnd: [255, 0, 0, 255],
    nextCell: [17, 34, 51, 255], lastCell: [17, 34, 51, 255],
  });
